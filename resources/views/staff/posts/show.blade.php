    ...
    <div class="bg-white p-6 rounded-lg shadow-lg">
        <div class="flex justify-between items-start mb-4">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('staff.posts.edit', $post) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    Edit
                </a>
                <form action="{{ route('staff.posts.destroy', $post) }}" method="POST" class="delete-form">
                    @csrf
                    @method('DELETE')
        ...
        <div class="mt-6">
            <a href="{{ route('staff.posts.index') }}" class="text-blue-500 hover:underline">Back to all posts</a>
        </div>
    </div>
@endsection
