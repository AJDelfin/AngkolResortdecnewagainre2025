...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl font-headline mb-6">Update Post Information</h2>
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="8" required
                              class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">{{ old('content', $post->content) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    @if ($post->image_path)
                        <div class="mt-4">
                            <img src="{{ asset('storage/' . $post->image_path) }}" alt="Current Image" class="h-32 w-auto object-cover rounded-md">
                        </div>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <div class="mt-2 flex items-center">
                        <input type="radio" name="visibility" id="visibility_public" value="public" @checked(old('visibility', $post->visibility) == 'public') class="mr-2">
                        <label for="visibility_public" class="mr-4">Public</label>
                        <input type="radio" name="visibility" id="visibility_private" value="private" @checked(old('visibility', $post->visibility) == 'private') class="mr-2">
                        <label for="visibility_private">Private</label>
                    </div>
                </div>

                <!-- User Selection -->
                <div id="user_selection" class="{{ old('visibility', $post->visibility) == 'private' ? '' : 'hidden' }}">
                        ...
                        <div id="admin_list" class="p-4">
                            @foreach ($admins as $user)
                                <div class="flex items-center">
                                    <input type="checkbox" name="users[]" value="{{ $user->id }}" id="user_{{ $user->id }}" @checked(in_array($user->id, old('users', $post->users->pluck('id')->toArray()))) class="mr-2">
