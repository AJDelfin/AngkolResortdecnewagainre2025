...
@section('content')
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('staff.reservations.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <div>
                        <input type="radio" id="type_room" name="reservation_type" value="room" checked class="mr-2">
                        <label for="type_room" class="mr-4">Room</label>
                        <input type="radio" id="type_cottage" name="reservation_type" value="cottage" class="mr-2">
                        <label for="type_cottage">Cottage</label>
                    </div>
                </div>

                <!-- Room Selection -->
                <div id="room_fields" class="mb-4">
                    ...
                    <select name="room_id" id="room_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        ...
                        @foreach ($rooms as $room)
                            <option value="{{ $room->id }}" data-price="{{ $room->price_per_night }}">{{ $room->room_number }} ({{ $room->name }}) - (₱{{ $room->price_per_night }})</option>
                        @endforeach
                    </select>
                </div>

                <!-- Cottage Selection -->
                <div id="cottage_fields" class="mb-4 hidden">
                    ...
                    <select name="cottage_id" id="cottage_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        ...
                        @foreach ($cottages as $cottage)
                            <option value="{{ $cottage->id }}" data-price="{{ $cottage->price_per_night }}">{{ $cottage->name }} - (₱{{ $cottage->price_per_night }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="number_of_adults" id="number_of_adults" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required min="0">
                </div>
                <div class="mb-4">
                    <label for="number_of_kids" class="block text-gray-700 text-sm font-bold mb-2">Number of Kids:</label>
                    <input type="number" name="number_of_kids" id="number_of_kids" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required min="0">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="total_price" id="total_price" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" readonly>
                </div>
                 <!-- Price Breakdown -->
